...
<body>
    ...
    if (isset($_POST['add_question'])) {
        ...
        $sql = "INSERT INTO loksewa(id, question, option1, option2, option3, option4, answer) 
                VALUES ('$id', '$question', '$option1', '$option2', '$option3', '$option4', '$answer')";
        if (mysqli_query($conn, $sql)) {
            echo "Question added successfully!<br>";
            echo "<script>window.location.href='addlevelone.php';</script>";
        } else {
            echo "Error: " . $sql . "<br>" . mysqli_error($conn);
        }
    }

    $sql = "SELECT * FROM loksewa";
...
if (mysqli_num_rows($result) > 0) {
  ...
  while ($row = mysqli_fetch_assoc($result)) {
    echo "<tr><td>" . $row["id"] . "</td><td>" . $row["question"] . "</td><td>" . $row["option1"] . "</td><td>" . $row["option2"] . "</td><td>" . $row["option3"] . "</td><td>" . $row["option4"] . "</td><td>" . $row["answer"] . 
        "</td><td><a href='edit1.php?id=" . $row["id"] . "'>Edit</a> | 
        <a href='delete1.php?id=" . $row["id"] . "' onclick=\"return confirm('Are you sure you want to delete this question?')\">Delete</a></td></tr>";
        }
        echo "</table>";
    } else {
